...
<body>
    ...
    <div class="info">
        ...
        <div><strong>Nama Pelanggan:</strong> {{ $transaction->customer->customerName ?? 'Umum' }}</div>
        <div><strong>Kasir:</strong> {{ $transaction->users->userName ?? '-' }}</div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $detail->motor->motorName ?? 'Produk tidak ditemukan' }}</td>
                <td>Rp {{ number_format( $detail->motor->price, 0, ',', '.')}}</td>
